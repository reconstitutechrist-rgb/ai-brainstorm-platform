...
export interface GeneratedDocument {
  id: string;
  project_id: string;
  document_type: 'project_brief' | 'decision_log' | 'rejection_log' | 'technical_specs' | 'project_establishment' | 'rfp' | 'implementation_plan' | 'vendor_comparison';
  title: string;
  content: string;
  version: number;
  created_at: string;
...
export class GeneratedDocumentsService {
  ...
  async generateDocuments(projectId: string): Promise<GeneratedDocument[]> {
    ...
    // Generate all five document types
...
Write this as a professional, well-structured document that could be shared with stakeholders.`;
