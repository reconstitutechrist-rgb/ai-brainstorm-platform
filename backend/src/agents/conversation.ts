...
MUST start your response with: "You're right, I misunderstood. [correct understanding]"
Do NOT ask questions when being corrected - just acknowledge and restate correctly.` : ''}
...
${referenceStr ? '\nIf they mention uploaded files: Reflect that: "You shared [filename] showing..." but do NOT analyze or suggest based on references unless asked.' : ''}`,
    ...
    if (violatesRules || questionCount > 1) {
...
Generate a simple, clean response that follows these rules EXACTLY.`,
      ...
      return {
        ...
        metadata: {
          wasRetried: true,
          originalViolation: violatesRules ? 'forbidden phrases' : 'multiple questions',
        },
      };
    }
