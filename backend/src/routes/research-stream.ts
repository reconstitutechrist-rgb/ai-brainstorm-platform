...
router.post('/live', async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Perform research with streaming callbacks
      ...
      },
      {
        ...
        onCrawlComplete: async (count: number) => {
          sendEvent('crawl_complete', {
            message: `Crawled ${count} sources successfully`,
            count,
            progress: 50,
          });
        },
        ...
        onQualityAssessment: async (count: number) => {
          sendEvent('quality_assessment', {
            message: `Assessed quality of ${count} sources`,
            count,
            progress: 60,
          });
        },
      ...
      result: {
        ...
        sources: result.sources.map(s => ({
          ...
          qualityScore: s.qualityScore ? {
            overall: s.qualityScore.overall,
            breakdown: s.qualityScore.breakdown,
            flags: s.qualityScore.flags,
          } : undefined,
        })),
