...
import { Send, Upload, Loader2, Bot, User as UserIcon, X } from 'lucide-react';
...
export const ChatPage: React.FC = () => {
  ...
  const loadMessages = useCallback(async () => {
    ...
    try {
      ...
      // On error, keep existing messages from localStorage
    }
  }, [currentProject, setMessages]);

  // Load messages and session data on project load
  useEffect(() => {
    if (currentProject && user) {
      // Only load messages if we haven't loaded them yet for this project
  ...
  }, [currentProject?.id, user?.id, loadMessages, loadAllSessionData]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Track activity after successful message
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <motion.div
        ...
        className={`${isDarkMode ? 'glass-dark' : 'glass'} rounded-3xl shadow-glass h-[calc(100vh-20rem)] flex flex-col`}
      >
        {/* Chat Header */}
