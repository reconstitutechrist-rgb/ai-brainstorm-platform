...
interface ProjectState {
  ...
  addItems: (items: import('../types').ProjectItem[]) => void;
  updateItems: (items: import('../types').ProjectItem[]) => void;
  // Selection actions
...
export const useProjectStore = create<ProjectState>((set, get) => ({
  ...
  addItems: (items) => set((state) => {
    if (!state.currentProject || items.length === 0) return state;

    const updatedItems = [...state.currentProject.items, ...items];
    const updatedProject = { ...state.currentProject, items: updatedItems };
  ...
  updateItems: (items) => set((state) => {
    ...
    const updatedItems = state.currentProject.items.map(item =>
      itemsMap.has(item.id) ? { ...item, ...itemsMap.get(item.id) } : item
    );
